...
def preprocessing():
    ...
    #Original database
    original_database = np.loadtxt('../File/in_file.mvect', delimiter=",", skiprows=1)
    ...
    ch_database = np.loadtxt(FILE_DB_PREFIX + INPUT_DB_CH, delimiter=",")

    sf_database = np.loadtxt(FILE_DB_PREFIX + INPUT_DB_SIFT, delimiter=",")

    #Creating video name to video num index and reverse index
    fileIndex = np.genfromtxt('../File/in_file.index', delimiter="=", dtype=None, skip_header=1)
